...
export default async function orderShippedHandler({
  ...
  while (attempts < MAX_RETRIES) {
    try {
      ...
      await resendNotificationService.sendOrderShippedEmail(orderId, email);
      console.log(`Order shipped email sent successfully for order ID: ${orderId}`);
      break; // Exit the loop if successful
    } catch (error) {
      attempts += 1;
      console.error(`Attempt ${attempts} failed to send order shipped email for order ID: ${orderId}`, error);
      if (attempts >= MAX_RETRIES) {
        console.error(`Failed to send order shipped email after ${MAX_RETRIES} attempts for order ID: ${orderId}`);
